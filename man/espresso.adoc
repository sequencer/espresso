...
*-Dmapdc*::
  Derive from the binary-valued variable _DONT_ CARE_ a don't-care set,
  and then delete this variable. All input conditions for which an
  output changes when _DONT_ CARE_ changes define the don't-care
...
*-estrong*::
  Uses the alternate strategy SUPER_ GASP (as a replacement for LAST_
